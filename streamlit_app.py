...
st.set_page_config(page_title="KS Distance to Brownian Motion", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 KS Distance to Brownian Motion</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Kolmogorov-Smirnov test | Macro‑dependent Brownian parameters | Deviation from randomness</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 KS Test")
...
st.sidebar.markdown(f"**All macros used**")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'ks_brownian_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Deviation score: {etf['ks_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized KS Statistic"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized KS Statistic", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by KS Deviation from Brownian Motion (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Kolmogorov-Smirnov test** compares the empirical return distribution to a Brownian motion (normal) distribution.
        - The Brownian parameters (drift μ, volatility σ) are adjusted using the composite macro factor.
        - The score is the KS statistic (max distance between empirical and theoretical CDFs).
        - High score → returns deviate strongly from Brownian motion → structured/predictable → potential alpha.
        - Low score → returns are close to a random walk (unpredictable).
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the KS scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("KS Distance to Brownian Motion | Macro‑adjusted random walk deviation")
